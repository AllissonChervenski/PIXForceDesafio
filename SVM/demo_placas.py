...
def load_sample(sample_fname, mode='train'):
...
def save_patches(src_path, w, step):
    ...
    for i, train_name in enumerate(train_names):
        ...
        img, gt, _ = load_sample(os.path.join(src_path, train_name))
...
fnames = os.listdir(src_path)
snames = [fname.split('.')[0] for fname in fnames if fname.endswith('.png')]
max_points = 0
...
for sname in snames:
   ...
   # plt.show()
    numtabuas_max=0
##################################################################################    
###faz conv para cada coluna da img 
    ninteracoes=len(edges[0])
    for nlinhas in range(300,ninteracoes-300):
     ...
     z = np.where(y==1)[0]
     h = abs(np.convolve(z, [-1, 1]))
    
##################################################################################
###faz um historiograma da coluna convoluida 
     h[0]=0 
     h[len(w)-1]=0
     counts, bins, patches = plt.hist(w)
   
################################################################################## 
#usa vetor logico para contar o numero de recorencias de linhas e pega o segundo maior
#valor do hist para fazer como parametro
     numlinhas=second_largest_number(counts)  
     lih=list(counts)
     lih=lih.index(numlinhas)
     parametro=bins[lih]/2
     
     logicalvector= w>=parametro   
     numlinhas=sum(bool(x) for x in logicalvector)

     numtabuas=numlinhas
     if numtabuas>numtabuas_max:
      numtabuas_max=numlinhas
##################################################################################      
###resultado final da img
    print('id da placa:',sname)
    print('numero de placas real:',len(points))
    print('numero de placas contadas:',numtabuas_max) 
##################################################################################
